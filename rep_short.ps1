# PowerShell script to replace standalone acronyms with \acrshort{} commands
...
function Parse-Acronyms {
    ...
    foreach ($line in $content) {
        # Match \newacronym{key}{ACRONYM}{Full Definition} with optional parameters
        if ($line -match '\\newacronym(?:\[([^\]]+)\])?\{([^}]+)\}\{([^}]+)\}\{([^}]+)\}') {
            $options = $matches[1]
            $key = $matches[2]
            $acronym = $matches[3]
            ...
            # Create pattern to match just the standalone acronym
            # We need to be careful about word boundaries to avoid partial matches
            $pattern = [regex]::Escape($acronym)
            ...
            Write-Host "Added pattern for '$key': $acronym" -ForegroundColor Green
            
            # Check if there's a plural form defined in the options
            if ($options -and $options -match 'plural=([^,\]]+)') {
                $pluralForm = $matches[1]
                $pluralPattern = [regex]::Escape($pluralForm)
                
                $acronyms[$pluralPattern] = @{
                    Key = $key
                    Acronym = $pluralForm
                    FullDef = $fullDef
                    Original = $pluralForm
                ...
                Write-Host "Added plural pattern for '$key': $pluralForm" -ForegroundColor Cyan
            }
        }
    }
...
function Process-File {
    ...
    foreach ($pattern in $sortedPatterns) {
        ...
        # Use appropriate command based on whether it's plural or not
        $replacement = if ($acronymInfo.IsPlural) {
            "\acrshortpl{$key}"
        } else {
            "\acrshort{$key}"
        }
        ...
        # This pattern ensures we only match standalone acronyms, not within other words
        ...
        $regexPattern = if ($ExcludeSections) {
            ...
            # URLs, file paths, but allow acronyms that are not already in braces
            "(?m)^(?!.*\\(?:caption|chapter|section|subsection|subsubsection|paragraph|subparagraph|title|author)\b)(.*)(\b$pattern\b)(?!\w)(?!\})(.*)$"
        } else {
            # Simple pattern that just matches standalone acronyms with word boundaries
            "(?m)(.*)(\b$pattern\b)(?!\w)(?!\})(.*)$"
        }
        ...
        # Filter out matches that shouldn't be replaced
        $validMatches = @()
        foreach ($match in $matches) {
            ...
            if (-not $shouldSkip) {
                ...
                foreach ($cmdMatch in $acronymCommands) {
                    ...
                    $acronymPosInLine = $line.IndexOf($pattern, [System.StringComparison]::OrdinalIgnoreCase)
...
# Main execution
Write-Host "=== Acronym Short Form Replacement Script ===" -ForegroundColor Cyan
...
if ($DryRun) {
    ...
    Write-Host "  .\rep_short.ps1                    # Process all chapter files" -ForegroundColor Gray
    Write-Host "  .\rep_short.ps1 -DryRun           # Preview changes without making them" -ForegroundColor Gray
    Write-Host "  .\rep_short.ps1 -ShowDetails      # Show detailed processing information" -ForegroundColor Gray
    Write-Host "  .\rep_short.ps1 -ExcludeSections:`$false  # Include section headers" -ForegroundColor Gray
}
