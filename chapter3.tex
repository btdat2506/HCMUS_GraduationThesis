...
The primary conduit for communication between the Rocket Core and the Gemmini accelerator is the RoCC (Rocket Custom Coprocessor) interface. This is not merely an external bus; it is a deep, co-design methodology that fundamentally alters the CPU's own hardware pipeline. When the \texttt{WithGemmini} trait is active, the Chipyard generator does more than just instantiate the accelerator—it injects specific logic into the Rocket Core's decoder, pipeline control, and hazard units. This section examines the specific code-level modifications that enable this tight coupling.
...
The integration begins at the earliest stage of the CPU pipeline: instruction decoding. The RISC-V ISA reserves four instruction opcode spaces for custom extensions. The Gemmini accelerator is assigned the \texttt{custom3} opcode space. The effect of this is visible directly in the Rocket Core's main decoder table, which is modified by the generator as shown in Listing \ref{lst:rocc_idecode}.

\begin{lstlisting}[language=Scala, caption={Modification of the Rocket Core's instruction decode table to recognize \texttt{custom3} opcodes. Source: \texttt{rocket-chip/src/main/scala/rocket/IDecode.scala}}, label=lst:rocc_idecode]
// From IDecode.scala - CPU recognizes custom3 instructions
CUSTOM3-> List(Y,N,Y,N,N,N,N,N,A2_ZERO,A1_RS1, IMM_X, DW_XPR, ...),
\end{lstlisting}

This line in the decoder's lookup table instructs the hardware that any instruction with the \texttt{custom3} opcode is a valid instruction that uses the register file (\texttt{A1\_RS1} specifies operand source) but does \textit{not} use the standard ALU. This prevents the CPU from treating it as an illegal instruction. Instead, it is flagged internally as a \texttt{rocc} command, which propagates down the pipeline stages.

\subsubsection{Pipeline Integration and Command Dispatch}
Once an instruction is flagged as a RoCC command, the CPU pipeline is modified to dispatch it via a dedicated interface in the final writeback stage. Instead of performing a standard register write, the hardware packages the necessary context into a \texttt{RoCCCommand} bundle (Listing \ref{lst:rocc_command_bundle}). The code in Listing \ref{lst:rocc_dispatch} shows this dispatch logic.

\begin{lstlisting}[language=Scala, caption={RoCC command dispatch logic in the Rocket Core's writeback stage. Source: \texttt{rocket-chip/src/main/scala/rocket/RocketCore.scala}}, label=lst:rocc_dispatch]
// RoCC command dispatch in the writeback (wb) stage
io.rocc.cmd.valid := wb_reg_valid && wb_ctrl.rocc && !replay_wb_common
io.rocc.cmd.bits.inst := wb_reg_inst
io.rocc.cmd.bits.rs1 := wb_reg_wdata
io.rocc.cmd.bits.rs2 := mem_reg_rs2
io.rocc.cmd.bits.status := csr.io.status
\end{lstlisting}

This Chisel code generates hardware where the \texttt{io.rocc.cmd.valid} signal is asserted only when a valid RoCC instruction reaches the writeback (`wb`) stage. Simultaneously, the hardware populates the \texttt{RoCCCommand} bundle with the instruction itself (\texttt{wb\_reg\_inst}), data from the CPU's register file (\texttt{wb\_reg\_wdata}, \texttt{mem\_reg\_rs2}), and the processor's status register. This direct, low-latency pathway from the CPU's internal pipeline registers to the accelerator interface is the defining feature of the RoCC's high-performance design.

\subsubsection{Hazard Management and Pipeline Stalls}
This tight integration requires new pipeline stall logic to handle hazards. The generator modifies the CPU's control unit to stall the pipeline if the accelerator is not ready to accept a command. This is shown in Listing \ref{lst:rocc_stall}.

\begin{lstlisting}[language=Scala, caption={RoCC-specific stall and replay logic in the Rocket Core. Source: \texttt{rocket-chip/src/main/scala/rocket/RocketCore.scala}}, label=lst:rocc_stall]
// Logic to determine if a command in the writeback stage needs to be replayed
val replay_wb_rocc = wb_reg_valid && wb_ctrl.rocc && !io.rocc.cmd.ready

// Add the RoCC stall condition to the main pipeline control stall signal
val ctrl_stalld = id_ctrl.rocc && io.rocc.busy || ...
\end{lstlisting}

The first line, \texttt{replay\_wb\_rocc}, generates hardware that detects if a RoCC command is in the writeback stage but the accelerator's command queue is full (\texttt{!io.rocc.cmd.ready}). If this condition is met, the entire pipeline is stalled, and the instruction is "replayed" until the accelerator is ready. The second line adds a condition to the main pipeline stall signal (\texttt{ctrl\_stalld}) that freezes instruction dispatch if a RoCC instruction is decoded in the ID stage while the accelerator is already busy processing a previous command (\texttt{io.rocc.busy}). These hardware interlocks are essential for correct synchronization.

\subsubsection{Data Response Path}
Finally, the RoCC interface provides a direct path for the accelerator to write results back into the CPU's register file. This is arbitrated with other units like the divider.

\begin{lstlisting}[language=Scala, caption={RoCC response handling in the Rocket Core, writing directly to the register file port. Source: \texttt{rocket-chip/src/main/scala/rocket/RocketCore.scala}}, label=lst:rocc_response]
...
As shown in Listing \ref{lst:rocc_response}, when the accelerator's response is valid (\texttt{io.rocc.resp.fire}), the hardware routes the response data (\texttt{io.rocc.resp.bits.data}) directly to the register file's write data port (\texttt{ll\_wdata}) and asserts the write enable signal (\texttt{ll\_wen}). This demonstrates a privileged, low-latency write path that completely bypasses the memory system, a feature impossible to achieve with a standard memory-mapped peripheral.

\subsubsection{RoCC Command Bundle}
...
In summary, these code-level modifications demonstrate that RoCC integration is a profound act of hardware co-design. It reconfigures the CPU's instruction decoder, enhances its pipeline stall logic, and creates specialized pathways for low-latency data and control exchange, proving the tight coupling that is essential for high-performance acceleration.

With the hardware co-design methodology of the RoCC interface established as the mechanism for integrating our accelerator, it is now necessary to understand why such specialized hardware is required in the first place. The next chapter will explore the architectural foundations of Deep Neural Network (DNN) acceleration, examining the core computational challenges and design principles that motivate the architecture of accelerators like Gemmini.