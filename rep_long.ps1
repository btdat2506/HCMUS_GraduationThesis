# PowerShell script to replace standalone acronym long forms with \acrlong{} commands
...
function Parse-Acronyms {
    ...
    foreach ($line in $content) {
        ...
        if ($line -match '\\newacronym(?:\[([^\]]+)\])?\{([^}]+)\}\{([^}]+)\}\{([^}]+)\}') {
            ...
            # Create pattern to match just the full definition (long form)
...
function Process-File {
    ...
    foreach ($pattern in $sortedPatterns) {
        ...
        $regexPattern = if ($ExcludeSections) {
            ...
            # URLs, file paths, but allow long forms that are not already in braces or parentheses
            "(?m)^(?!.*\\(?:caption|chapter|section|subsection|subsubsection|paragraph|subparagraph|title|author)\b)(.*)(\b$pattern\b)(?!\w)(?!\})(?!\))(.*)$"
        } else {
            # Simple pattern that just matches standalone long forms with word boundaries
            "(?m)(.*)(\b$pattern\b)(?!\w)(?!\})(?!\))(.*)$"
        }
        
        # Additional check to avoid replacing long forms that are part of compound words or in specific contexts
        ...
        foreach ($match in $matches) {
            ...
            # Smart filtering: Only skip if the long form is in specific problematic contexts
            $shouldSkip = $false
            $skipReason = ""
            ...
            if (-not $shouldSkip) {
                # Look for patterns like "Full Definition (ACRONYM)" where this might be the full definition part
                $lineStartPos = $content.LastIndexOf("`n", $match.Index) + 1
                $matchPosInLine = $match.Groups[2].Index - $lineStartPos
                $matchEndInLine = $matchPosInLine + $match.Groups[2].Length
                ...
                if ($matchEndInLine -lt ($line.Length - 1) -and $line[$matchEndInLine] -eq ' ' -and ($matchEndInLine + 1) -lt ($line.Length - 1) -and $line[$matchEndInLine + 1] -eq '(') {
                    # Check if what follows looks like an acronym in parentheses
                    $afterMatch = $line.Substring($matchEndInLine)
                    if ($afterMatch -match '^\s+\([A-Z0-9\-]+\)') {
                        $shouldSkip = $true
                        $skipReason = "Part of full definition with acronym in parentheses"
                    }
                }
            }
...
Write-Host "Found $($acronyms.Count) acronym long form patterns to replace" -ForegroundColor Green
Write-Host ""
...
if ($DryRun) {
    ...
    Write-Host "  .\rep_long.ps1                     # Process all chapter files" -ForegroundColor Gray
    Write-Host "  .\rep_long.ps1 -DryRun            # Preview changes without making them" -ForegroundColor Gray
    Write-Host "  .\rep_long.ps1 -ShowDetails       # Show detailed processing information" -ForegroundColor Gray
    Write-Host "  .\rep_long.ps1 -ExcludeSections:`$false  # Include section headers" -ForegroundColor Gray
}
