...
The result shows that the overhead of using \texttt{clock\_gettime(CLOCK\_MONO\allowbreak TONIC)} is the lowest, around 7.4 us, while \texttt{gettimeofday} has a similar overhead of around 7.7 us. The \texttt{clock\_gettime\allowbreak (CLOCK\_\allowbreak PRO\allowbreak CESS\_CPUTIME\_ID)} has a higher overhead of around 26.4 us, which is expected as it provides more detailed CPU time information.
...
void test_timing_demo() {
    ...
    printf("Results:\n");
    printf("  clock_gettime(CLOCK_MONOTONIC):     %lu ns (%.2f us)\n", 
           end_mono - start_mono, (end_mono - start_mono) / 1000.0);
    printf("  gettimeofday():                     %lu us (%.2f us)\n", 
           end_gtod - start_gtod, (double)(end_gtod - start_gtod));
    printf("  clock_gettime(CLOCK_PROCESS_CPUTIME): %lu ns (%.2f us)\n", 
...
Results:
  clock_gettime(CLOCK_MONOTONIC):     69000 ns (69.00 us)
  gettimeofday():                     69 us (69.00 us)
  clock_gettime(CLOCK_PROCESS_CPUTIME): 128000 ns (128.00 us)
  Sum result: 499500
\end{lstlisting}
