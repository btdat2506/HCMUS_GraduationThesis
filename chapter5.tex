...
% Gemmini is an open-source, full-stack DNN accelerator \textit{generator} developed at the \acrlong{ucb} \cite{gemini-dac}. It is designed not merely as a standalone hardware component, but as a complete ecosystem for exploring and evaluating domain-specific hardware acceleration. 
...
Gemmini integrates into the Chipyard SoC via the \textbf{\acrfull{rocc}} interface (Section \ref{sec:rocketchip_soc_structure}). As a tightly-coupled coprocessor, it receives custom RISC-V instructions dispatched directly by the host Rocket Core's pipeline. This allows for low-latency command and control.
